...
struct VulkanXcbRenderer::Impl {
    ...
    HINSTANCE hinst=nullptr; HWND hwnd=nullptr; bool resizePending=false; bool closeRequested=false;
    HMODULE vkLib=nullptr; PFN_vkGetInstanceProcAddr gip=nullptr; PFN_vkGetDeviceProcAddr gdp=nullptr;
    ...
    PFN_vkDestroyInstance DestroyInstance{}; PFN_vkCreateWin32SurfaceKHR CreateSurface{}; PFN_vkDestroySurfaceKHR DestroySurface{};
    ...
    static LRESULT CALLBACK wndProc(HWND h,UINT msg,WPARAM wp,LPARAM lp){
        Impl* self=reinterpret_cast<Impl*>(GetWindowLongPtrW(h,GWLP_USERDATA));
        if(msg==WM_NCCREATE){auto* cs=reinterpret_cast<CREATESTRUCTW*>(lp);self=static_cast<Impl*>(cs->lpCreateParams);SetWindowLongPtrW(h,GWLP_USERDATA,reinterpret_cast<LONG_PTR>(self));if(self)self->hwnd=h;}
        if(!self)return DefWindowProcW(h,msg,wp,lp);
        switch(msg){
            case WM_CLOSE:self->closeRequested=true;return 0;
            case WM_DESTROY:self->closeRequested=true;PostQuitMessage(0);return 0;
            case WM_SIZE:{uint32_t w=LOWORD(lp),hh=HIWORD(lp);if(w&&hh&&(w!=self->width||hh!=self->height)){self->width=w;self->height=hh;self->resizePending=true;}return 0;}
            case WM_MOUSEMOVE:{float sc=std::min(float(self->width)/RenderFrame::LogicalWidth,float(self->height)/RenderFrame::LogicalHeight);float ox=(float(self->width)-RenderFrame::LogicalWidth*sc)*0.5f,oy=(float(self->height)-RenderFrame::LogicalHeight*sc)*0.5f;self->input.mouseX=(float(GET_X_LPARAM(lp))-ox)/sc;self->input.mouseY=(float(GET_Y_LPARAM(lp))-oy)/sc;self->input.mouseInside=self->input.mouseX>=0&&self->input.mouseY>=0&&self->input.mouseX<RenderFrame::LogicalWidth&&self->input.mouseY<RenderFrame::LogicalHeight;return 0;}
            case WM_LBUTTONDOWN:self->input.mouseDown=true;self->input.mousePressed=true;SetCapture(h);return 0;
            case WM_LBUTTONUP:self->input.mouseDown=false;self->input.mouseReleased=true;ReleaseCapture();return 0;
            case WM_KILLFOCUS:self->clearKeyboard();self->input.mouseDown=false;return 0;
            case WM_KEYDOWN:case WM_SYSKEYDOWN:if((lp&(1ll<<30))==0)self->keyEvent(unsigned(wp),true);return 0;
            case WM_KEYUP:case WM_SYSKEYUP:self->keyEvent(unsigned(wp),false);return 0;
            default:return DefWindowProcW(h,msg,wp,lp);
        }
    }
    void makeWindow(const std::string& title){
        hinst=GetModuleHandleW(nullptr);const wchar_t* cls=L"HeartGoldNativeVulkanWindow";WNDCLASSEXW wc{};wc.cbSize=sizeof(wc);wc.style=CS_HREDRAW|CS_VREDRAW|CS_OWNDC;wc.lpfnWndProc=&Impl::wndProc;wc.hInstance=hinst;wc.hCursor=LoadCursor(nullptr,IDC_ARROW);wc.lpszClassName=cls;RegisterClassExW(&wc);
        int n=MultiByteToWideChar(CP_UTF8,0,title.c_str(),-1,nullptr,0);std::wstring wt(std::max(1,n),L'\0');MultiByteToWideChar(CP_UTF8,0,title.c_str(),-1,wt.data(),n);
        RECT r{0,0,LONG(width),LONG(height)};AdjustWindowRect(&r,WS_OVERLAPPEDWINDOW,FALSE);hwnd=CreateWindowExW(0,cls,wt.c_str(),WS_OVERLAPPEDWINDOW|WS_VISIBLE,CW_USEDEFAULT,CW_USEDEFAULT,r.right-r.left,r.bottom-r.top,nullptr,nullptr,hinst,this);if(!hwnd)throw std::runtime_error("CreateWindowExW failed");ShowWindow(hwnd,SW_SHOW);UpdateWindow(hwnd);std::cout<<"Controller: Windows build currently uses keyboard/mouse input\n";
    }
    void loadInstanceFns(){DestroyInstance=load_i<PFN_vkDestroyInstance>(gip,instance,"vkDestroyInstance");CreateSurface=load_i<PFN_vkCreateWin32SurfaceKHR>(gip,instance,"vkCreateWin32SurfaceKHR");DestroySurface=load_i<PFN_vkDestroySurfaceKHR>(gip,instance,"vkDestroySurfaceKHR");EnumeratePhysicalDevices=load_i<PFN_vkEnumeratePhysicalDevices>(gip,instance,"vkEnumeratePhysicalDevices");GetQProps=load_i<PFN_vkGetPhysicalDeviceQueueFamilyProperties>(gip,instance,"vkGetPhysicalDeviceQueueFamilyProperties");GetMemoryProps=load_i<PFN_vkGetPhysicalDeviceMemoryProperties>(gip,instance,"vkGetPhysicalDeviceMemoryProperties");GetSurfaceSupport=load_i<PFN_vkGetPhysicalDeviceSurfaceSupportKHR>(gip,instance,"vkGetPhysicalDeviceSurfaceSupportKHR");GetCaps=load_i<PFN_vkGetPhysicalDeviceSurfaceCapabilitiesKHR>(gip,instance,"vkGetPhysicalDeviceSurfaceCapabilitiesKHR");GetFormats=load_i<PFN_vkGetPhysicalDeviceSurfaceFormatsKHR>(gip,instance,"vkGetPhysicalDeviceSurfaceFormatsKHR");GetPresentModes=load_i<PFN_vkGetPhysicalDeviceSurfacePresentModesKHR>(gip,instance,"vkGetPhysicalDeviceSurfacePresentModesKHR");CreateDevice=load_i<PFN_vkCreateDevice>(gip,instance,"vkCreateDevice");gdp=load_i<PFN_vkGetDeviceProcAddr>(gip,instance,"vkGetDeviceProcAddr");}
    void loadDeviceFns(){
...
#undef LD
    ...
    void initVulkan(){
        vkLib=LoadLibraryA("vulkan-1.dll");if(!vkLib)throw std::runtime_error("vulkan-1.dll not found - install a Vulkan-capable GPU driver");gip=reinterpret_cast<PFN_vkGetInstanceProcAddr>(GetProcAddress(vkLib,"vkGetInstanceProcAddr"));if(!gip)throw std::runtime_error("vkGetInstanceProcAddr missing");auto CreateInstance=reinterpret_cast<PFN_vkCreateInstance>(gip(nullptr,"vkCreateInstance"));if(!CreateInstance)throw std::runtime_error("vkCreateInstance missing");
        VkApplicationInfo ai{VK_STRUCTURE_TYPE_APPLICATION_INFO,nullptr,"HeartGold Native Port",30,"HG Native",30,VK_API_VERSION_1_0};const char* exts[]={VK_KHR_SURFACE_EXTENSION_NAME,VK_KHR_WIN32_SURFACE_EXTENSION_NAME};VkInstanceCreateInfo ci{VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,nullptr,0,&ai,0,nullptr,2,exts};vkcheck(CreateInstance(&ci,nullptr,&instance),"vkCreateInstance");loadInstanceFns();VkWin32SurfaceCreateInfoKHR sci{VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR,nullptr,0,hinst,hwnd};vkcheck(CreateSurface(instance,&sci,nullptr,&surface),"vkCreateWin32SurfaceKHR");
    ...
    static bool keyMap(unsigned code,GameButton& out){switch(code){
        case 'W':case VK_UP:out=GameButton::Up;return true;case 'S':case VK_DOWN:out=GameButton::Down;return true;case 'A':case VK_LEFT:out=GameButton::Left;return true;case 'D':case VK_RIGHT:out=GameButton::Right;return true;
        case 'E':case 'Z':case VK_RETURN:case VK_SPACE:out=GameButton::Interact;return true;case VK_ESCAPE:case 'X':out=GameButton::Menu;return true;case VK_SHIFT:out=GameButton::Run;return true;
        case VK_F5:out=GameButton::Save;return true;case VK_F9:out=GameButton::Load;return true;case VK_F3:out=GameButton::Debug;return true;case VK_F2:out=GameButton::Assets;return true;case VK_F4:out=GameButton::Terrain;return true;case 'R':out=GameButton::Reset;return true;case 'Q':out=GameButton::Quit;return true;default:return false;}}
    void keyEvent(unsigned code,bool down){GameButton b;if(!keyMap(code,b))return;auto i=static_cast<std::size_t>(b);keyboardDown[i]=down;refresh(b);}
    ...
    void destroyStaging(){
        if(dev&&stagingMapped&&stagingMemory&&UnmapMemory)UnmapMemory(dev,stagingMemory);
        stagingMapped=nullptr;
        if(dev&&stagingBuffer&&DestroyBuffer)DestroyBuffer(dev,stagingBuffer,nullptr);
        if(dev&&stagingMemory&&FreeMemory)FreeMemory(dev,stagingMemory,nullptr);
        stagingBuffer=0;stagingMemory=0;stagingSize=0;
    }
...
int VulkanXcbRenderer::run(NativeGame& game){bool running=true;auto last=std::chrono::steady_clock::now();while(running&&!game.wantsQuit()&&!p->closeRequested){p->input.clearEdges();MSG msg{};while(PeekMessageW(&msg,nullptr,0,0,PM_REMOVE)){if(msg.message==WM_QUIT){running=false;break;}TranslateMessage(&msg);DispatchMessageW(&msg);}p->pollJoystick();auto now=std::chrono::steady_clock::now();double dt=std::chrono::duration<double>(now-last).count();last=now;game.update(p->input,dt);if(!running||game.wantsQuit()||p->closeRequested)break;try{p->draw(game.render());}catch(const std::exception& e){std::cerr<<e.what()<<"\n";return 2;}}return 0;}
